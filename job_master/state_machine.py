...
import asyncpg
import httpx
from worker_spawner import spawn_mapper, spawn_reducer, delete_worker_job
...
class JobStateMachine:
    ...
    async def _create_map_tasks(self):
        ...
        for i in range(num_mappers):
            ...
            await self.db.execute(
                ...
                ON CONFLICT (job_id, map_id) DO NOTHING
                """,
    ...
    async def _spawn_and_track_mapper(self, map_id: int):
        ...
        if previous_attempt > 0:
            # Tear down the prior attempt's K8s Job (and its pod) so a slow or
            # zombie worker cannot keep running and overwrite/duplicate shards.
            try:
                await asyncio.to_thread(
                    delete_worker_job, "mapper", self.job_id, map_id, previous_attempt
                )
            except Exception as exc:
                logger.warning(
                    f"[{self.job_id}] Failed to delete stale mapper_{map_id} "
                    f"attempt {previous_attempt}: {exc}"
                )
    ...
    async def _spawn_and_track_reducer(self, reduce_id: int):
        ...
        if previous_attempt > 0:
            try:
                await asyncio.to_thread(
                    delete_worker_job, "reducer", self.job_id, reduce_id, previous_attempt
                )
            except Exception as exc:
                logger.warning(
                    f"[{self.job_id}] Failed to delete stale reducer_{reduce_id} "
                    f"attempt {previous_attempt}: {exc}"
                )
        task.attempt_number += 1
        task.status          = TASK_STATUS_RUNNING
        task.started_at      = datetime.now(timezone.utc)
        output_path = f"{self.job['output_data_path']}part_{reduce_id}.json"
        await self.db.execute(
