...
async def create_job_record(user_id: str, job_req: JobSubmissionRequest) -> UUID:
    ...
    """
    job_id = uuid4()
    # Canonical, collision-free per-job storage layout. The intermediate prefix
    # is aligned with what the worker pods actually use for shuffle shards
    # (intermediate/{job_id}/...), and the output prefix is predictable so the
    # CLI can fetch results by job_id alone.
    intermediate_prefix = f"intermediate/{job_id}/"
    output_data_path = f"users/{user_id}/jobs/{job_id}/output/"
    async with _pool.acquire() as conn:
        async with conn.transaction():
            ...
            """, job_id, user_id, job_req.input_data_path, output_data_path,
